...
const ConsultantCropUpdates = () => {
  ...
  // Fetch all crop updates on component mount
  useEffect(() => {
    fetchCropUpdates();
  }, []);

  // Fetch crop updates from API
  const fetchCropUpdates = async () => {
    try {
      const response = await fetch(`${API_URL}/crop-updates/${userData.id}`);
  ...
  const createCropUpdate = async (e) => {
    e.preventDefault();
    const payload = {
      ...formData,
    ...
    try {
      const response = await fetch(`${API_URL}/crop-updates`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      if (response.ok) {
        await fetchCropUpdates();
        setFormData({
          date: "",
          unique_id: "",
          name_of_consultant: "",
      ...
      console.error("Error creating crop update:", error);
    }
  };

  // Open edit modal with existing crop update data
  const openEditModal = (update) => {
    if (update.status !== "Pending") {
      alert("Crop update is not in pending status");
      return;
    }
  ...
  // Update an existing crop update
  const updateCropUpdate = async (e) => {
    e.preventDefault();
    const payload = {
      ...formData,
      consultant_id: userData.id,
    ...
    try {
      const response = await fetch(`${API_URL}/crop-updates/update`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      ...
      console.error("Error updating crop update:", error);
    }
  };
      ...
      <div className="py-4 flex justify-between items-center">
        <div>
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-700">
            Below are the recent updates for crops in the respective regions.
          </p>
        </div>
        <button
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded shadow-lg dark:bg-gray-800 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={createCropUpdate}>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded shadow-lg dark:bg-gray-800 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={updateCropUpdate}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                    Date
...
export default ConsultantCropUpdates;